...
final class TraceIndexer {

  private static final Logger LOG = LoggerFactory.getLogger(TraceIndexer.class);
  private final PreparedStatement prepared;
  private final TimestampCodec timestampCodec;
  private final String boundName;
  private final IndexSupport index;
  /** Shared across all threads, as updates to indexes can come from any thread. */
  private final ConcurrentMap<PartitionKeyToTraceId, Pair<Long>> sharedState;
  @Nullable
  private final Integer indexTtl;
  private final Session session;

  TraceIndexer(Session session, TimestampCodec timestampCodec,
      ConcurrentMap<PartitionKeyToTraceId, Pair<Long>> sharedState,
      IndexSupport index, @Nullable Integer indexTtl) {
    this.index = index;
  ...
  ImmutableSet<ListenableFuture<?>> index(List<Span> spans) {
    ...
    ImmutableSetMultimap<PartitionKeyToTraceId, Long> toInsert =
        entriesThatIncreaseGap(maybeInsert);
  ...
  ImmutableSetMultimap<PartitionKeyToTraceId, Long> entriesThatIncreaseGap(
    ...
    return entriesThatIncreaseGap(sharedState, updates);
  }
  ...
  static class Factory {
    ...
    public Factory(Session session, TimestampCodec timestampCodec,
        ConcurrentMap<PartitionKeyToTraceId, Pair<Long>> sharedState, @Nullable Integer indexTtl) {
      this.session = session;
      this.timestampCodec = timestampCodec;
      this.sharedState = sharedState;
      this.indexTtl = indexTtl;
    ...
    TraceIndexer create(IndexSupport index) {
      return new TraceIndexer(session, timestampCodec, sharedState, index, indexTtl);
    }
  }
}
